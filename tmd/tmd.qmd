...
# tmd file names
# tnames <- c("masterpr139", 
#             "issue126_4017a0", 
#             "my126_ratio", 
#             "my126_ratio_imp0",
#             "my126_ratio_imp.01v2",
#             "my126_ratio_saltfix",
#             "my126_ratio_imp.1",
#             "my126_ratio_imp0001")
# 
# tnames <- c("issue126_4017a0",
#             "my126_ratio",
#             "my126_ratio_imp0", 
#             "my126_ratio_imp.01v2",
#             "my126_ratio_imp0001")
# 
# tnames <- c("issue126_4017a0", # 0.2
#             "my126_ratio",     # 0.2
#             "my126_ratio_imp0", 
#             "my126_ratio_imp.01v2",
#             "my126_ratio_imp0001",
...
(tfiles <- paste0("tmd_2021_", tnames, ".csv"))
tpaths <- fs::path(scratch_path, tfiles)
# stack1 <- vroom(tpaths, id = "source")

get_file <- function(tname){
  print(tname)
...
stack0 <- purrr::map(tnames, get_file) |> list_rbind()
ns(stack0)
count(stack0, tmd)
...
stack |> 
  summarise(ctc=sum(ctc_total * s006), .by=c(tmd, data_source))

stack |> 
  summarise(ctc=sum(ctc_total * s006), .by=c(tmd))


stack |> 
  mutate(ratio = s006 / s006_original) |> 
  filter(data_source==1) |> 
  summarise(ssd=sum((ratio - 1)^2), .by=tmd) |> 
  arrange(ssd)
...
df |> 
  filter(tmd=="masterpr139") |> 
  mutate(tmd="original", weight=s006_original) |> 
  bind_rows(df |> 
              rename(weight=s006)) |> 
  ggplot(aes(weight)) +
  geom_histogram(bins = 60, fill = "blue", color = "black") +
...
# tpath <- fs::path(stor_path, "tmd_2021.csv")
# tpath <- fs::path(stor_path, "tmd_2021.csv")
# tpath <- fs::path(scratch_path, "tmdout-21-#-#-#.csv")
# tpath <- fs::path(scratch_path, "master_pr134", "tmd_2021.csv")
# tpath <- fs::path(scratch_path, "issue126", "tmd_2021.csv")

tmdtc2021 <- vroom(tpath) # if run through tc by Nikhil
