...
ptargets2 <- ptargets |> 
  ...
  mutate(nranges=n(), .by=c(table, datatype, year, vnbase, vtype, irsvname))
...
vars <- c("c00100", "c01000neg", "c01000pos", "c02500", "c04800", "e00200", "e00300", "e00400", "e00600", "e00900neg", "e00900pos",  "e001500", "e002300", "e002400",
          "e17500", "e18400")
...
tmd2 <- tmdtc2021 |>
  ...
  mutate(c01000neg = c01000 * (c01000 < 0) * sign(c01000),
         c01000pos = c01000 * (c01000 > 0) * sign(c01000)) |> 
  mutate(e00900neg = e00900 * (e00900 < 0) * sign(e00900),
         e00900pos = e00900 * (e00900 > 0) * sign(e00900)) |> 
...
sums_ycut19 <- tmd_ycut19 |> 
  summarise(value=sum(value), .by=c(weightvar, tmdname)) |> 
  mutate(incsort=1)
...
comp <- tmd4 |> 
  inner_join(uxtargets,
            by = join_by(tmdname, incsort, year, datatype)) |> 
  select(contains("name"), vnbase, datatype, vtype, table, weightvar, incsort, incrange, ptarget, value) |> 
...
dollcols <- c("ptarget", "value", "diff", "absdiff")
tab <- comp |> 
  mutate(across(any_of(dollcols),
                \(x) ifelse(vtype=="amount", x / 1e9, x))) |> # $ billions
  select(weightvar, vtype, vnbase, tmdname, irsvname, incsort, incrange, ptarget:abspdiff) |> 
  arrange(weightvar, vtype, vnbase, tmdname, incsort) |> 
  DT::datatable(rownames = FALSE,
            options = list(scrollX = TRUE, scrollY = '1800px', pageLength = 20),
            filter="top") |>
    formatCurrency(columns = dollcols, currency="", digits=2) |>
