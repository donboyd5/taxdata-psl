...
get_colmap <- function(tab){
  ...
  col_map <- read_excel(path(dd, targfn), sheet=sheet, skip=3) |> 
    ...
    select(table, year, column_num, column_letter, colname, description, units, pufvar) |> 
    filter(!is.na(column_letter), !is.na(colname)) |> 
...
get_irsdata <- function(fname, startrow, endrow, maxcol, column_letters, colnames){
...
ptargets_nested <- tabget |> 
  # filter(row_number() == 1) |> 
  mutate(targets=get_irsdata(fname, startrow, endrow, maxcol, column_letters, colnames) |> list(), .by=fname)
ptargets_nested
ptargets_nested |> unnest(col=targets)

ptargets <- ptargets_nested |> 
  select(table, year, fname, table_description, targets) |> 
  unnest(col=targets) |> 
...
datatable(ptargets, options = list(scrollX = TRUE, scrollY = '400px', pageLength = 50)) |> 
