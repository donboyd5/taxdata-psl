...
dfl |> 
  ...
  fmt_number(columns=c(starts_with("n_"), starts_with("wtsum_")),
             # scale=1e-9,
             decimals=0) |> 
    fmt_number(columns=-c(wcut, starts_with("n_"), starts_with("wtsum_")),
             scale=1e-9,
             decimals=2) |> 
...
-   We need to penalize changes in weights. There is information in the PUF weights that is being discarded by the PolicyEngine approach to weights, which does not penalize changes in weights. Right now, the optimization routine just goes in and selects weights that hit targets, without worrying about whether weights change a lot thereby creating innocent bystanders.
...
dfl |> 
  ...
  fmt_number(columns=c(starts_with("n_"), starts_with("wtsum_")),
             # scale=1e-9,
             decimals=0) |> 
    fmt_number(columns=-c(MARS, starts_with("n_"), starts_with("wtsum_")),
             scale=1e-9,
             decimals=2) |> 
