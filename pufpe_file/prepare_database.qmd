...
## Create arrow database out of zipped csv files
...
df1 |>
  group_by(year) |>
  write_dataset(path = dpq, format = "parquet", existing_data_behavior = "overwrite")
b <- proc.time()
b - a # 24 secs

# how big? about same as .gz files
tibble(
  files = list.files(dpq, recursive = TRUE),
  size_MB = file.size(file.path(dpq, files)) / 1024^2
)
